...
if(typeof msg.text !== "string"){
  ...
  if(history===undefined || history===null){
    ...
    console.log("here 2")
    await client.set(String(chatId),JSON.stringify([msg.text + res.text]), { EX: 900000 });//15min
   }else{
    console.log("message"+message)
    console.log("history"+history)
    const res = await chain.call({ question: message, chat_history: history });
    // send back the matched "whatever" to the chat
    console.log("response of llm  "+ JSON.stringify(res))
    bot.sendMessage(chatId, res.text);
    await client.set(String(chatId), JSON.stringify([...history, msg.text + res.text])); 
   }
